...
% sample size and number of factors (covariates) in the vector entering
% the conditioning set of the conditional copula
n = 500; p = 30;

% Sparsity degree, that is the proportion of zero coefficients in beta, the
% vector of coefficients for the linear combination beta * covariates
...
for oo = 1:Nsim
    ...
    for kk = 1:len_scad
        ...
        % MSE
        MSE(oo,kk) = vec(beta_true-beta_est_scad)'*vec(beta_true-beta_est_scad);
    ...
    for kk = 1:len_mcp
        ...
        % MSE
        MSE(oo,len_scad+kk) = vec(beta_true-beta_est_mcp)'*vec(beta_true-beta_est_mcp);
    ...
    MSE(oo,len_scad+len_mcp+1) = vec(beta_true-beta_est_lasso)'*vec(beta_true-beta_est_lasso);
    toc
end
